...
class ModelPredictor(BaseEstimator, TransformerMixin):
    def __init__(self, modelwithparams=None, random_number=-1, problem_type='infer'):
        ...
        modelwithparams: a list with two entries. The first is a string denoting the model to use. The second is a dict, which is a parameter distribution of the related model. Default = None, which means a random forest classifier with default setting.
        ...
        pipe = Pipeline([("null", br.RemoveAllNull()),                   
                 ...
                 ("imp", br.ImputeData(columns_must_impute='x4')),
        ...
        rand_num = br.NumberGeneratorForModelPredictor(422)
        ...
        if self.max_features != 'all':
            ...
            if prob_type == "regression":
                ...
                if self.method == "RandomForest":
                    ...
                    self.model = self.method
            self.model.fit(X[:rows_to_scan_in], y[:rows_to_scan_in])
            results = pd.Series(self.model.feature_importances_, X.columns).order(ascending=False)
    ...
    def fit(self, X, y):
        ...
        if prob_type == "regression":
            ...
            if self.method == "RandomForest":
                ...
                self.model = self.method
        sampleX = X[:rows_to_scan_in]
        sampley = y[:rows_to_scan_in]
        for col in X.columns:
...
### 8/25/2015 AJR added some specialized classes into the package
